...
base_path = '/n/contefs1/backup2/colenso/';
exp_folder = [base_path '06_17_15_sample2/elastic_align/'];
path2 = [exp_folder  'conv_merged_ds/'];
convfiles = dir([path2 '*.png']);
convnames = convfiles.name;
...
%The number of core used can be changed here for different computer. 
poolobj = gcp('nocreate');
if isempty(poolobj)
    parpool(32)
end

%Use the processed storm images B*4 as masks on the conventional images. 
parfor j=1:num_images2
    disp('Processing image j') 
    A = (imread([path2 convnames(sec_use(j))]));
    A1 = A(:,:,1);
    A2 = A(:,:,2);
%    A3 = A(:,:,3);
    %figure;
...
varuse1(idx,:) = mean(currparam)*ones(1,num_images2);
...
varuse2(idx,:) = mean(currparam)*ones(1,num_images2);
...
%%
%stormnames???convnames???
info = imfinfo([path2 stormfiles(sec_use(1),1).name]);
    width = info.Width;
    height = info.Height;
...
%stormnames???convnames???
A = (imread([path2 stormfiles(sec_use(i),1).name]));
    A1 = A(:,:,1);
    A2 = A(:,:,2);
    A3 = A(:,:,3);
...
figure;
imshow(uint8(temp1*413*4))
figure;
imshow(uint8(temp2*413*7))
    %%
...
varuse1s(idx,:) = mean(currparam)*ones(1,num_images2);
...
varuse2s(idx,:) = mean(currparam)*ones(1,num_images2);
...
%%
path9 = [base_path '06_17_15_sample2/elastic_thresh/'];
if exist(path9)~=7
    mkdir(path9);
...
dim = 1;
B1 = logical(A1);
C1 = squeeze(sum(B1,dim))+1;
temp1 = squeeze(mean(A1,dim))./C1;
temp1a = squeeze(mean(A1,dim));
B2 = logical(A2);
C2 = squeeze(sum(B2,dim))+1;
temp2 = squeeze(mean(A2,dim))./C2;
temp2a = squeeze(mean(A2,dim));
%%
figure;
imshow(uint8(temp1a*13*4))
%
figure;
imshow(uint8(temp2a*13*5))
%%
figure;
subplot(1,3,1)
imshow(squeeze(max(A2,[],dim))*1)
subplot(1,3,2)
imshow(uint8(temp2a*num_images2*0.1))
subplot(1,3,3)
imshow(uint8(temp2*num_images2*10))
savefig([path9 'gephyrin_storm_cross_section_hist_normalization_1st_dim.fig'])