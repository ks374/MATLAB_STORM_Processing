base_folder = 'Z:\Chenghang\chenghaz_014_XB2_P2_Control_B\analysis\Result\';
outpath = base_folder;
outpath2 = 'Z:\Chenghang\chenghaz_014_XB2_P2_Control_B\analysis\Result\Rand\';

%%
load([outpath2 'statsR_n_rand.mat']);
...
for i = 1:numel(statsGwater_n_random)
    centRs(i,1) = statsGwater_n_random(i).WeightedCentroid(1) * voxel(1);
    centRs(i,2) = statsGwater_n_random(i).WeightedCentroid(2) * voxel(2);
    centRs(i,3) = statsGwater_n_random(i).WeightedCentroid(3) * voxel(3);
end
volumeRs = [statsGwater_n_random.Area]';

load([outpath 'statslistG2nsw10.mat']);
load([outpath2 'statsG_n_rand.mat']);
voxel = [15.5,15.5,70];
%%
for i = 1:numel(statsGwater_n_random)
    centGs(i,1) = statsGwater_n_random(i).WeightedCentroid(1) * voxel(1);
    centGs(i,2) = statsGwater_n_random(i).WeightedCentroid(2) * voxel(2);
    centGs(i,3) = statsGwater_n_random(i).WeightedCentroid(3) * voxel(3);
end
%
...
volumeGs = [statsGwater_n_random.Area]';
...
%
save([base_folder 'nearest_neightbor_pairing_randg_r.mat'],'nn_*')
%%
binsl = 0:0.05:5;
...
plot(hx2,hy4,'b'); alpha(0.5)

savefig([base_folder 'nnrandG_Rs_counts_with_rand_and_all.fig'])
%%
...
plot(hx2,hy8,'b'); alpha(0.5)

savefig([base_folder 'nnRsplit_randGs_counts_with_rand_and_all.fig'])
...
%%
load([outpath2 'add_to_rand_statsGw10_edges.mat'],'tintsG_p70');

mints_g70s = (([tintsG_p70])./[volumeGs]');
%%
...
Eps = 2.07; clustID = 1; classg = zeros(numel(RDg),1);
for i = 1:numel(RDg)
    if RDg(orderg(i))>Eps
        if CDg(orderg(i))<=Eps
...
savefig([outpath2 'storm_randgs_ps_mints_nn_shell_2d_hist.fig'])
...
%
save([outpath2 'pairing_index_ps_randgs_withedges.mat'],'pairedg_idx')
...
load([outpath2 'statsG_n_rand.mat']);
%
statsGwater = statsGwater_n_random;
statsGwater_s = statsGwater(pairedg_idx);
statsGwater_n = statsGwater(~pairedg_idx);
save([outpath2 'randG_paired.mat'],'statsGwater_s','statsGwater_n');
%%
clear tintsG_p210 mints_g210s
load([outpath2 'add_to_rand_statsRw10_edges.mat'],'tintsG_p70');

mints_R70s = (([tintsG_p70])./[volumeRs]');
%%
Xn=70; Yn=80; Xrange=[-0.2 3]; Yrange=[1.2 4.0];
Xlo = Xrange(1) ; Xhi = Xrange(2) ; Ylo = Yrange(1) ; Yhi = Yrange(2) ; 
X = linspace(Xlo,Xhi,Xn)' ; Y = linspace(Ylo,Yhi,Yn)' ;
...
figure; plot(CDg(orderg))
%% get dbscan clusters from optics threshold and plot (geph)
Eps = 1.5; clustID = 1; classg = zeros(numel(RDg),1);
for i = 1:numel(RDg)
    if RDg(orderg(i))>Eps
        if CDg(orderg(i))<=Eps
            clustID = clustID + 1; classg(orderg(i)) = clustID;
...
h2 = line(xval, yval);
set(h2,'Color','r','LineWidth',2)
savefig([outpath2 'storm_rs_randgs_mints_nn_shell_2d_hist.fig'])
%%
pairedg_idx = -((L(1)/L(2))*val3w + K/L(2))>val4w;
numel(find(pairedg_idx))/numel((pairedg_idx))
save([outpath2 'pairing_index_rs_randgs_withedges.mat'],'pairedg_idx')
%%

load([outpath2 'statsR_n_rand.mat']);
statsGa2s = statsGwater_n_random;
statsRwater_s = statsGa2s(pairedg_idx);
statsRwater_n = statsGa2s(~pairedg_idx);
save([outpath2 'randR_paired.mat'],'statsRwater_s','statsRwater_n');
