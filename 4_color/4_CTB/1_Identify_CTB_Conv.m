...
%
DYs_b = zeros(size(DYs),'uint8');
parfor k=1:num_images
    disp(k)
    DYs_b(:,:,k) = (imfilter(DYs_8(:,:,k),fspecial('gaussian',gausspix*2,gausspix),'same','replicate'));
end

DYs_3B = imfilter(DYs_b,Hz,'same','replicate');
clear DYs
DYs = logical(DYs_3B);
%
Sel_Conv = I; 
Sel_Conv = I.*uint8(DYs);
%
a = Sel_Conv(:);
a(a==0) = [];
LH = stretchlim(a,0.005);
Sel = uint8(zeros(size(Sel_Conv)));
parfor i =1:num_images
    Sel(:,:,i) = imadjust(Sel_Conv(:,:,i),LH,[]);
end

%%
% i = 1
%     imwrite(Sel(:,:,i),[outpath sprintf('%03d',i) '.tif']);
%
