...
%%
load([base_folder 'sizeshapematG_and_cent_water10_area_cutoff.mat']);
% load([base_folder 'statslistR2nsw10.mat']);

%%
voxel = [15.5 15.5 70];
% sizeshape_matGns = sizeshape_matGa2ns;%(rcentGa2ns(:,2)<46000,:); % centGa2ns2 = centGa2ns;%(rcentGa2ns(:,2)<46000,:);
% centGns = rcentGa2ns;%(rcentGa2ns(:,2)<46000,:);
centG = centGw;
sizeshape_matGs = sizeshape_mat;%(rcentGa2s(:,2)<46000,:);
centGa2s2 = centG;%(rcentGa2s(:,2)<46000,:);
centGs = centG.*voxel;%(rcentGa2s(:,2)<46000,:);

sizeshape_matGns = sizeshape_matGs;
centGns = centGs;
%%
...
%%
save([base_folder 'nearest_neightbor_pairing_gw10vw10.mat'],'nn_*')
%%
binsl = 0:0.05:5;
...
savefig([base_folder 'nnGsplit_Vs_counts_with_rand_and_all.fig'])
%%
%figure;
...
savefig([base_folder 'nnVsplit_Gs_counts_with_rand_and_all.fig'])
%
%figure;
...
%%
load([base_folder 'add_to_statsGw10_edges.mat'],'tintsG_p280');

mints_g70s = (([tintsG_p280])./[volumeGs]');
%%
...
Eps = 0.0795; clustID = 1; classg = zeros(numel(RDg),1);
for i = 1:numel(RDg)
    if RDg(orderg(i))>Eps
        if CDg(orderg(i))<=Eps
...
savefig([base_folder 'storm_gs_vs_mints_nn_shell_2d_hist.fig'])
...
%
save([base_folder 'pairing_index_gs_vs_withedges.mat'],'pairedg_idx')
...
load([outpath_v 'add_to_statsVw10_edges.mat'],'tintsG_p280');

mints_R70s = (([tintsG_p280])./[volumeRs]');
%%
Xn=70; Yn=80; Xrange=[-0.2 3]; Yrange=[1.2 4.0];
Xlo = Xrange(1) ; Xhi = Xrange(2) ; Ylo = Yrange(1) ; Yhi = Yrange(2) ; 
X = linspace(Xlo,Xhi,Xn)' ; Y = linspace(Ylo,Yhi,Yn)' ;
...
% figure; plot(CDg(orderg))
%% get dbscan clusters from optics threshold and plot (geph)
Eps = 0.368; clustID = 1; classg = zeros(numel(RDg),1);
for i = 1:numel(RDg)
    if RDg(orderg(i))>Eps
        if CDg(orderg(i))<=Eps
            clustID = clustID + 1; classg(orderg(i)) = clustID;
...
h2 = line(xval, yval);
set(h2,'Color','r','LineWidth',2)
savefig([base_folder 'storm_vs_gs_mints_nn_shell_2d_hist.fig'])
%%
pairedg_idx = -((L(1)/L(2))*val3w + K/L(2))>val4w;
numel(find(pairedg_idx))/numel((pairedg_idx))
save([base_folder 'pairing_index_vs_gs_withedges.mat'],'pairedg_idx')
%%

load([outpath_v 'statsV2sw10.mat']);
statsVwater_ss = statsGa2s(pairedg_idx);
statsVwater_sn = statsGa2s(~pairedg_idx);
save([outpath_v 'VG_paired.mat'],'statsVwater_ss','statsVwater_sn');
