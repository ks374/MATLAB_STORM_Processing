base_folder = 'Z:\Chenghang\chenghaz_005_Sample_P8_C_A\analysis\elastic_align\Result\';
outpath = base_folder;
...
load([outpath2 'statsG_rand.mat']);
load([outpath2 'statsG_n_rand.mat']);
voxel = [15.5,15.5,70];
%%
for i = 1:numel(statsGwater_n_random)
    centGns(i,1) = statsGwater_n_random(i).WeightedCentroid(1) * voxel(1);
    centGns(i,2) = statsGwater_n_random(i).WeightedCentroid(2) * voxel(2);
    centGns(i,3) = statsGwater_n_random(i).WeightedCentroid(3) * voxel(3);
end

for i = 1:numel(statsGwater_random)
    centGs(i,1) = statsGwater_random(i).WeightedCentroid(1) * voxel(1);
    centGs(i,2) = statsGwater_random(i).WeightedCentroid(2) * voxel(2);
    centGs(i,3) = statsGwater_random(i).WeightedCentroid(3) * voxel(3);
end
%
...
volumeGs = [statsGwater_random.Area]';
...
%%
load([outpath2 'add_to_rand_statsGw10_edges.mat'],'tintsG_p140');
...
Eps = 0.31; clustID = 1; classg = zeros(numel(RDg),1);
for i = 1:numel(RDg)
    if RDg(orderg(i))>Eps
        if CDg(orderg(i))<=Eps
...
load([outpath2 'statsG_rand.mat']);
%
statsGwater = statsGwater_random;
statsGwater_ss = statsGwater(pairedg_idx);
statsGwater_sn = statsGwater(~pairedg_idx);
save([outpath2 'randG_paired.mat'],'statsGwater_ss','statsGwater_sn');
%%
clear tintsG_p210 mints_g210s
load([outpath2 'add_to_rand_statsRw10_edges.mat'],'tintsG_p140');
...
figure; plot(CDg(orderg))
%% get dbscan clusters from optics threshold and plot (geph)
Eps = 0.5; clustID = 1; classg = zeros(numel(RDg),1);
for i = 1:numel(RDg)
    if RDg(orderg(i))>Eps
        if CDg(orderg(i))<=Eps
            clustID = clustID + 1; classg(orderg(i)) = clustID;
...
load([base_folder 'statsR2sw10.mat']);
statsRwater_ss = statsGa2s(pairedg_idx);
statsRwater_sn = statsGa2s(~pairedg_idx);
save([outpath2 'randR_paired.mat'],'statsRwater_ss','statsRwater_sn');
